export function Thankyou() {
  return (
    <div className="main-div flex flex-col px-20 flex-wrap w-full items-center ">
      <div className="main-div mx-[379px] mt-[173px]">
        <div className="w-[682px] h-[311px] ">
          <p className="h-[39px] font-semibold text-[32px] text-center">
            Thank you for creating an account!
          </p>
          <p className="mt-5 text-center font-medium text-[24px]">
            Your information is under review. We select startups very carefully,
            as we want to keep the quality top notch on Unbundl.
            <br /> You are going to get a confirmation email in the next 24
            hours. We might ask you to jump on a call with us, before approving
            your profile.
          </p>
        </div>
      </div>
    </div>
  );
}
